...
def check_system_status() -> Dict[str, Any]:
    """Check the status of all required system dependencies and configurations."""
    status = {"status": "operational", "components": {}, "errors": []}
    required_packages = ["reportlab", "python-docx", "beautifulsoup4", "markdown", "PyPDF2"]
    
    for package in required_packages:
        try:
            __import__(package.replace("-", "_"))
            status["components"][package] = "available"
        except ImportError as e:
            status["components"][package] = "missing"
            status["errors"].append(f"Missing required package: {package}")
    
    for directory in ["media/exports", "media/temp", "docs_output"]:
        dir_path = Path(directory)
        if not dir_path.exists():
            try:
                dir_path.mkdir(parents=True, exist_ok=True)
                status["components"][f"dir_{directory}"] = "created"
            except Exception as e:
                status["components"][f"dir_{directory}"] = "error"
                status["errors"].append(f"Could not create directory {directory}: {str(e)}")

    if status["errors"]:
        status["status"] = "degraded" if len(status["errors"]) < len(required_packages) else "error"
    
    status["available_formats"] = ["html", "markdown", "txt"]
    if all(status["components"].get(pkg, "") == "available" for pkg in ["reportlab", "PyPDF2"]):
        status["available_formats"].append("pdf")
    if status["components"].get("python-docx", "") == "available":
        status["available_formats"].append("docx")
...
class ProjectAnalyzer:
    ...
    def check_system_status(self) -> Dict[str, Any]:
        """
        Check the status of all required system dependencies and configurations.
        Returns a dictionary with system status information.
        """
        status = {
            "status": "operational",
            "components": {},
        ...
        for package in required_packages:
            try:
                ...
                status["components"][package] = "missing"
                status["errors"].append(f"Missing required package: {package}")
                if package in ["reportlab", "PyPDF2"]:
                    status["pdf_export"] = "unavailable"
                elif package == "python-docx":
                    status["docx_export"] = "unavailable"
        ...
        # Add available export formats
        status["available_formats"] = ["html", "markdown", "txt"]  # These are always available
        if "pdf_export" not in status:
            status["available_formats"].append("pdf")
        if "docx_export" not in status:
            status["available_formats"].append("docx")
